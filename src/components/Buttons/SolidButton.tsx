...
export function SolidButton({icon, children, ...rest} : ButtonModelProps){
    ...
    return (
        <Button px="8" w="fit-content" color="black" borderRadius="0" size="lg" fontWeight="semibold" leftIcon={icon && <Icon as={icon} stroke="#ffffff" fontSize="16" fill="none" mr="2"/>} variant="solid" fontSize="sm" _hover={{boxShadow: `0 8px 20px -8px ${boxShadowColor}`}} _active={{}} {...rest}>
            {children}
        </Button>
    )
}